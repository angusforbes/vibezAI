...
### Changed
- **SV and FE show no query text** — the saved lists and the feed take no text, so whatever was typed for Apple Music or Claude Code is hidden in the prompt row and the SEARCH footer while one of them is up, and shows again when `ctrl+/` comes back round to AM or CC. The text itself is kept.
- **Discover is `F`, and it is R for one song** — the similarity metric, its picker and the `:discover` commands are gone: they searched curated artist lists rather than asking Apple, so "same artist" did not mean that. `F` turns discover on and off, listed in the Tracks footer as `F discover` with `● on` while it runs. While on, every track that starts playing gets one song from its Apple Music station inserted right after it, once per track, so the next song is always something new and nothing already queued is lost. `+`/`-` are volume again in every state, and the library hint reads `T`, not `⇧T`.
- **About is credits plus one key** — while the About panel (`?`) is open only `esc`, `?` and `Tab` (close) and `ctrl+shift+d` do anything; the latter opens the original author's Ko-fi page in the browser and is listed in the About footer alone, which is the whole footer while About is open. Every other key is dropped there, so nothing reaches the player, the other panels or the command line from a screen of credits; the panel's header line and rule are gone, and so are its old Enter/`d` link and the `:donate` command. In Tracks, `shift+→` now moves the highlighted track down (later in play order) and `shift+←` up, following the arrows rather than the J/K letters.
- **Lyrics and Equalizer footers list only their own keys** — while either panel is open the footer is that panel's row alone, from either column; the playback keys still work there, they are just not repeated.
...
- **Search-only Find panel and random library picks** — the library browser panel (`l`) is no longer offered since search covers the library; `ctrl+shift+r` inserts five random, not-yet-queued songs from your library right after the highlighted track (the library list is fetched once and cached for ten minutes).
- **Engine queue kept in sync by track id** — the engine drops tracks it cannot play when it loads a queue and kept its own copy of the order, so index-based removes and moves drifted and "add and play" could land on a track that was no longer in the list (or a removed one, when the last track was deleted while playing). A new `SyncQueue` engine operation now rebuilds the engine's list from the on-screen queue after every structural edit, finds the playing track again by id, starts a requested track only once the list is in place, and stops when the playing track is gone. Album/playlist search results go up to 25 (Apple's maximum) instead of 10.
- **Section titles are selectable and a track is never queued twice** — Enter on a section title folds the section or opens it again; Tab and Enter skip tracks already in the queue (Tab highlights the queued copy, Enter plays it, collections add only their missing tracks).
- **Search sections reordered** — results are shown as Playlists, then Albums, then Library (tracks you own), then catalog Tracks, five items per section, so the narrow column stays scannable. A section shows a "+ 5 more" row while matches are hidden and a "− 5 less" row while any are shown (enter on them grows or shrinks the section in steps of five, down to nothing), and playlists take a single line with their track count inline.
- **Find panel** — the right column now holds Apple Music search (default, `/`) or the library (`l`) instead of the vibe prompt, and the queue stays visible while you browse; the vibe prompt and the discovery picker still borrow the column while they have focus (`v`, `:discover metric`). In both tabs `tab` adds the selection to the end of the queue without playing and `enter` adds it and starts it; `esc` hands the keys back to the queue and keeps the query and results. Search, library and feed "play" actions never replace the queue any more (library playlists are added through their track IDs). The search footer hints, `ctrl+r` (radio) and `ctrl+p` (playlist picker) are gone.
- **Space is only play/pause; Enter starts songs** — `space` never jumps to the highlighted entry any more; `enter` plays the highlighted entry and restarts the playing one when it is highlighted. The `f` favourite key and the heart icon next to the transport controls are disabled.
...
### Added
- **Navigate the queue from the main view** — the mini-queue under Now Playing has a cursor: `↑`/`↓` (or `k`/`j`) highlight an entry without touching playback, `space` or `enter` jumps to the highlighted track with the whole queue left in place (the queue panel's `enter` still drops the entries above), `d` (also `x`/`delete`) removes it, `K`/`J` (or `shift+←`/`shift+→`) move it, `gg`/`G` jump to the ends and `esc` drops the highlight so the list follows the playing track again. `n`/`p` still skip immediately, and every other key behaves exactly as before. The highlight is always present: it follows the playing track until you move it, `q` puts it back, and `esc` does nothing in the main view. `c` clears the queue, `shift+↑`/`shift+↓` jump the highlight to the top/bottom, `shift+d` removes the highlighted entry and everything below it, and `ctrl+shift+d` removes everything above it. Discovery mode moved off the `d` key (it is `F` now, see above).
