...
- **Search sections reordered** — results are shown as Playlists, then Albums, then Library (tracks you own), then catalog Tracks, five items per section, so the narrow column stays scannable. Sections with more matches end in a selectable "+ N more" row (enter expands it, then "− less" folds it), and playlists take a single line with their track count inline.
