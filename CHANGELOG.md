...
- **Search sections reordered** — results are shown as Playlists, then Albums, then Library (tracks you own), then catalog Tracks, five items per section, so the narrow column stays scannable. Every section ends in "+ 5 more" and "− 5 less" rows (enter on them grows or shrinks the section in steps of five, down to nothing), and playlists take a single line with their track count inline.
